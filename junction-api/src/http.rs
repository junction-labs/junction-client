...
pub struct Route {
    // fixme: here is where the gateway API allows a parentref, and we likely we
    // will need something like it
...
pub struct RouteRule {
    ...
    ///
    /// All filters are compatible with each other except for the
    /// URLRewrite and RequestRedirect filters, which may not be combined.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub filters: Vec<RouteFilter>,

    //todo: enable session persistence #[serde(default, skip_serializing_if
    // ="Option::is_none")] /pub session_persistence:
    ...
    //#[serde(default, skip_serializing_if = "Vec::is_empty")] pub backend_refs:
    // Vec<HTTPBackendRef>,
}

/// Defines timeouts that can be configured for a http Route. Timeout values are
/// represented with Gateway API Duration formatting. Specifying a zero value
/// such as "0s" is interpreted as no timeout.
#[derive(Serialize, Deserialize, Clone, Debug, JsonSchema, Eq, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
...
pub enum RouteFilter {
    /// D efines a schema for a filter that modifies request headers.
...
pub struct HeaderFilter {
    ...
    ///
    /// If multiple entries specify equivalent header names, the first entry
    /// with an equivalent name will be considered for a match. Subsequent
...
pub enum PathModifier {
    ...
    /// to "/foo/bar" with a prefix match of "/foo" and a ReplacePrefixMatch of
...
pub struct RequestRedirectFilter {
    ...
    /// header in the response. When empty, the hostname in the `Host` header of
    /// the request is used.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Port is the port to be used in the value of the `Location` header in the
    /// response.
    ///
    ...
    ///   Listener port.
    ///
    /// Implementations SHOULD NOT add the port number in the 'Location' header
...
// FIXME: This is what the CRD requires, and I think its likely what our API
// will require. Instead for now though we use the xDS centric "target" concept
//
// #[derive(Serialize, Deserialize, Clone, Debug, JsonSchema, Eq, PartialEq)]
// #[serde(rename_all = "camelCase")] pub struct FilteredBackendRef {
// #[serde(flatten)] pub backend_ref: Option<BackendRef>, #[serde(default,
//     skip_serializing_if = "Vec::is_empty")] pub filters: Vec<RouteFilter>, }
#[derive(Serialize, Deserialize, Clone, Debug, JsonSchema, Eq, PartialEq)]
...
///
/// ( Modelled on the forthcoming Gateway API
...
pub struct SessionAffinityHashParam {
    pub r#type: SessionAffinityHashParamType,
    pub name: String,
    ...
    fn from_xds(header_matcher: &xds_route::HeaderMatcher) -> Option<Self> {
        ...
        let (r#type, value) = match header_matcher.header_match_specifier.as_ref()? {
            ...
            xds_route::header_matcher::HeaderMatchSpecifier::SafeRegexMatch(pfx) => {
                //fixme: we should validate engine type and somewhow record if
...
impl PathMatch {
    fn from_xds(path_spec: &xds_route::route_match::PathSpecifier) -> Option<Self> {
        return match path_spec {
            xds_route::route_match::PathSpecifier::Prefix(p) => Some(PathMatch {
                r#type: PathMatchType::PathPrefix,
                value: p.clone(),
            }),
            xds_route::route_match::PathSpecifier::Path(p) => Some(PathMatch {
                r#type: PathMatchType::Exact,
                value: p.clone(),
            }),
            xds_route::route_match::PathSpecifier::SafeRegex(p) => Some(PathMatch {
    ...
    pub fn from_xds(hash_policy: &xds_route::route_action::HashPolicy) -> Option<Self> {
        ...
        match hash_policy.policy_specifier.as_ref() {
            Some(PolicySpecifier::Header(h)) => Some(SessionAffinityHashParam {
                ...
                r#type: SessionAffinityHashParamType::Header,
...
mod tests {
    ...
    use crate::http::{RouteRule, SessionAffinityPolicy};
        ...
        assert_deserialize(
            ...
            Route {
                ...
                rules: vec![RouteRule {
                    ...
                    filters: vec![],
                    timeouts: None,
                    session_affinity: Some(SessionAffinityPolicy {
